...
We want to launch the `execl` function. Try to modify values on gdb doesn’t work as we then run `“/bin/sh”` as user `bonus3`.

To trigger it we need to satisfy the condition :

`strcmp(buffer, argv[1]) == 0` 

Looking at the source code, the second `fread` is in every case useless because we have set `buffer[59] = 0;` just before and the content that was read starts at `buffer[66]`. We only can affect :

`buffer[atoi(argv[1])] = 0;` 

We can try an empty `argv[1]` :
