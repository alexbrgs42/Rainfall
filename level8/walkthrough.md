...
Dump of assembler code for function main:
   ...
   0x08048574 <+16>:	nop
   0x08048575 <+17>:	mov    ecx,DWORD PTR ds:0x8049ab0           // service variable
   0x0804857b <+23>:	mov    edx,DWORD PTR ds:0x8049aac           // auth variable
   0x08048581 <+29>:	mov    eax,0x8048810                        // put "%p, %p\n" in eax
   ...
   0x08048596 <+50>:	mov    eax,ds:0x8049a80                     // put stdin in eax
...
Using Hex-Rays https://dogbolt.org/?id=c81dd233-7ccb-4ab8-8074-27857c96eb14#Boomerang=1&Hex-Rays=145 we obtain :
...
I noticed that if you call multiple times "auth ", its address does increase by of factor of 16 (0x10) each time :
...
And similarly with "service" on its dedicated address :
...
I want to trigger the instruction (reached after typing `"login"`) `if ( *((_DWORD *)auth + 8) )` in order to get a shell. In asm we can see that it actually is +0x20 → `if ( *((_DWORD *)auth + 0x20) )`.

If we init both `auth` and `service` , we see adjacent addresses :
...
We will try to init `auth` and then increase `service` until `auth + 0x20` is a non null char ! We can guess that it will be 2 times, because each call increases the address by 0x10 (16).
